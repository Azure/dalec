...
func createBuildScript(spec *dalec.Spec, opts ...llb.ConstraintsOpt) llb.State {
	...
	if spec.HasPips() {
		...
		fmt.Fprintln(buf, "      *.txt) python3 -m pip install --target=site-packages --find-links=\"${PIP_CACHE_DIR}\" --no-index --requirement=\"$reqfile\" 2>/dev/null || python3 -m pip install --target=site-packages --find-links=\"${PIP_CACHE_DIR}\" --no-index --requirement=\"$reqfile\" ;;")
		fmt.Fprintln(buf, "      *) python3 -m pip install --target=site-packages --find-links=\"${PIP_CACHE_DIR}\" --no-index . 2>/dev/null || python3 -m pip install --target=site-packages --find-links=\"${PIP_CACHE_DIR}\" --no-index . ;;")
