...
func createBuildScript(spec *dalec.Spec, opts ...llb.ConstraintsOpt) llb.State {
	...
	if spec.HasCargohomes() {
		...
		if hasCargoCache {
			...
			fmt.Fprintln(buf, "if [ -f /tmp/dalec/sccache ] && [ -x /tmp/dalec/sccache ]; then")
			fmt.Fprintln(buf, "  export RUSTC_WRAPPER=/tmp/dalec/sccache")
			fmt.Fprintln(buf, "  echo 'Using sccache for cargo build caching'")
			fmt.Fprintln(buf, "elif [ -f 'C:/temp/dalec/sccache.exe' ]; then")
			fmt.Fprintln(buf, "  export RUSTC_WRAPPER='C:/temp/dalec/sccache.exe'")
			fmt.Fprintln(buf, "  echo 'Using sccache for cargo build caching'")
