...
func SpecHandler(ctx context.Context, client gwclient.Client, spec *dalec.Spec, targetKey string) (*gwclient.Result, error) {
	sOpts, err := frontend.SourceOptFromClient(ctx, client)
	if err != nil {
		return nil, fmt.Errorf("failed to get sourceOptFromClient: %s", err)
	}
	st, err := Dalec2SpecLLB(spec, llb.Scratch(), targetKey, "", sOpts)
	if err != nil {
		return nil, err
	}
...
func Dalec2SpecLLB(spec *dalec.Spec, in llb.State, targetKey, dir string, sOpts dalec.SourceOpts, opts ...llb.ConstraintsOpt) (llb.State, error) {
	...
	if err := WriteSpec(spec, targetKey, buf, sOpts); err != nil {
