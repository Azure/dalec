...
func withTestDeps(w worker, spec *dalec.Spec, sOpt dalec.SourceOpts, targetKey string, opts ...llb.ConstraintsOpt) (llb.StateOption, error) {
	...
	withRepos, err := withRepoConfig(testRepos, sOpt, opts...)
	if err != nil {
		return nil, err
	}

	withData, err := withRepoData(testRepos, sOpt, opts...)
	if err != nil {
		return nil, err
	}

	keyMounts, keyPaths, err := withRepoKeys(testRepos, sOpt, opts...)
	if err != nil {
		return nil, err
	}
	...
	return func(in llb.State) llb.State {
		...
		depNames := dalec.SortMapKeys(deps)
		return base.Run(
			w.Install(depNames, atRoot("/tmp/rootfs"),
