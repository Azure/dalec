...
//
// Build target is an exact match to a registered route Build target is empty,
// check if a default handler is registered Check if any of the registered
// handlers have a route that is a prefix match to the build target BuildMux
// route handlers also must be buildkit BuildFuncs. As such a handler can itself
// also be a distinc BuildMux with its own set of routes.  This allows handlers
// to also do their own routing. All logic for what to route is handled outside
// of the BuildMux.
//
...
func (m *BuildMux) Add(targetKey string, bf gwclient.BuildFunc, info *bktargets.Target) {
	if m.handlers == nil {
...
func maybeSetDalecTargetKey(client gwclient.Client, key string) gwclient.Client {
	...
	return setClientOptOption(client, keyTargetOpt, key)
}
...
func (m *BuildMux) Handle(ctx context.Context, client gwclient.Client) (_ *gwclient.Result, retErr error) {
	...
	defer func() {
		if retErr != nil {
			if _, ok := origOpts[keyTargetOpt]; !ok {
	...
	client = maybeSetDalecTargetKey(trimTargetOpt(client, matched), matched)
