...
func (c *BazelCache) ToRunOption(worker llb.State, distroKey string, opts ...BazelCacheOption) llb.RunOption {
	return RunOptFunc(func(ei *llb.ExecInfo) {
		...
		// Add bazel-remote socket if it exists as a bazel "remote_cache"
		// llb.SSHOptional is used to not fail if the socket does not exist
		rcFile = worker.Run(
			ShArgs("[ -S "+sockPath+" ] || exit 0; echo 'build --remote_cache=unix:"+sockPath+"' >> /tmp/work/bazelrc"),
			llb.AddSSHSocket(llb.SSHID("bazel-remote"), llb.SSHSocketTarget(sockPath), llb.SSHOptional),
			llb.IgnoreCache,
		).AddMount("/tmp/work", rcFile)
		...
		llb.AddSSHSocket(llb.SSHID("bazel-remote"), llb.SSHSocketTarget(sockPath), llb.SSHOptional).SetRunOption(ei)
	})
}
