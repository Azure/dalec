...
func buildScript(spec *dalec.Spec) string {
	...
	if spec.HasPips() {
		...
		fmt.Fprintln(b, "      *.txt) python3 -m pip install --target=site-packages --find-links=\"${PIP_CACHE_DIR}\" --no-index --requirement=\"$reqfile\" 2>/dev/null || python3 -m pip install --target=site-packages --find-links=\"${PIP_CACHE_DIR}\" --no-index --requirement=\"$reqfile\" ;;")
		fmt.Fprintln(b, "      *) python3 -m pip install --target=site-packages --find-links=\"${PIP_CACHE_DIR}\" --no-index . 2>/dev/null || python3 -m pip install --target=site-packages --find-links=\"${PIP_CACHE_DIR}\" --no-index . ;;")
		fmt.Fprintln(b, "    esac")
		fmt.Fprintln(b, "  fi")
		fmt.Fprintln(b, "done")
		fmt.Fprintln(b, "")
