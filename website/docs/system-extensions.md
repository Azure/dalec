...
The above issues can be largely avoided by including statically-linked rather than dynamically-linked binaries. This will make your system extension much more portable. However, binaries shipped by distributions will practically always be dynamic, so this will require you to build the binaries and their dependencies yourself as part of the spec.

### Prefixing

Conflicts may still occur even when static linking. The extension should ideally place its files under a unique prefix, e.g. `/usr/local/my-sysext`. The standard layout may exist below this, e.g. `/usr/local/sysext/bin`. For this to work, the binaries should be built specifically for this prefix. Dynamically-linked binaries should explicitly include somewhere like `/usr/local/sysext/lib` in the `RUNPATH` to avoid the need for wrappers that set `LD_LIBRARY_PATH`.

### Permitted paths

System extensions will only merge files under `/usr` and `/opt`. Files located elsewhere will be not appear once merged. systemd supports configuration extensions, known as confexts, for handling `/etc`, but these are not yet supported by Dalec. In the meantime, anything installed under `/etc` by the spec is relocated under `/usr/share/<NAME>/etc`, and a tmpfiles.d entry is created to copy these files to `/etc` when the extension is merged. They are not removed when the extension is unmerged.
